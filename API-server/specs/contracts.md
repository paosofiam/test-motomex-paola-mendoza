...
- El chat activo más reciente se resuelve en `chat_service` con `ORDER BY created_at DESC LIMIT 1` (regla de un solo chat activo por lead; el soft-delete del previo lo hace `chat_service.create`).
- `lead_service.search` lista leads activos con filtros opcionales por `chat_whatsapp_id` e `intencion_de_compra` (string); respalda el endpoint `GET /leads`.

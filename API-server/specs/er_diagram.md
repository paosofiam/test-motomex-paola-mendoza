...
- Regla de negocio de leads: **solo puede existir un lead activo por `chat_whatsapp_id` a la vez**. `POST /leads` es **idempotente**: si ya hay un lead activo con ese `chat_whatsapp_id`, devuelve el existente (`200 OK`) sin crear otro; para enriquecerlo se usa `PATCH /leads/{id}`. Los leads **no se borran** vía API. La consulta `GET /leads?chat_whatsapp_id=` retorna a lo sumo un único lead (el más reciente, `created_at DESC LIMIT 1`).
- Regla de negocio de chats: **solo puede existir un chat activo por `chat_whatsapp_id` a la vez**. `POST /chats` es **idempotente**: si ya hay un chat activo con el mismo `lead_id` **o** `chat_whatsapp_id`, devuelve el existente (`200 OK`) sin crear ni borrar. Para reemplazar un chat, el previo debe eliminarse (soft delete) primero vía `DELETE` — `create` nunca lo elimina. Por esta razón, los endpoints de consulta de chats por `chat_whatsapp_id` o por `lead_id` deben retornar a lo sumo un único chat (el más reciente, `created_at DESC LIMIT 1`). El soft-delete de un chat además fija su `chat_status_id` en `5` (cerrado): borrar un chat equivale a terminar la sesión, así que la fila borrada queda cerrada y no como "activo" (los dos ejes —`deleted_at` y `chat_status_id`— quedan consistentes).
- Nota de implementación (leads y chats): la unicidad "uno activo por `chat_whatsapp_id`" se valida en la capa service (check-then-insert), no con un UNIQUE de BD, porque MySQL no soporta índice UNIQUE parcial sobre `WHERE deleted_at IS NULL` (un UNIQUE total chocaría con las filas soft-deleted). Queda una pequeña ventana TOCTOU bajo peticiones concurrentes del mismo `chat_whatsapp_id`.
...
4. **`Lead.chat_id` y `Lead.status` en la respuesta del endpoint NO son columnas de la tabla `leads`**. Son campos derivados/calculados del chat activo más reciente del lead (`chat_id` = su `id`; `status` = su `chat_status` resuelto a string), consistente con la regla "un chat activo por `chat_whatsapp_id`". `lead_service` los resuelve vía `resolvers.get_active_chat` (join a `chats` con `created_at DESC LIMIT 1` y `deleted_at IS NULL`). Importante no confundirlos con columnas en `leads`. (La respuesta también expone `lead_id` como alias informativo de `id`.)
